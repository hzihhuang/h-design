 ...
 * 用来为元素添加选中功能
 */
const CheckContainer = forwardRef<CheckContainerRef, CheckContainerProps>((props, ref) => {
  const {
    className = '',
    style,
    children,
  ...
  const typeElement = useMemo(() => {
    switch (type) {
      case 'radio':
        return <div className={classNames('radio-check', { checked: checked })}></div>;
      case 'checkbox':
        return <div className={classNames('checkbox-check', { checked: checked })}></div>;
      default:
        return null;
    }
  }, [type, checked]);
  ...
  const modeElement = useMemo(() => {
    ...
    switch (mode) {
      ...
      case 'checked':
        return (
          <div className="check-element" style={{ opacity: checked ? 1 : void 0 }}>
            {typeElement}
          </div>
        );
      // 一直存在
